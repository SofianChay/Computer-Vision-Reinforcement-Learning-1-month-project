...
class MultiExperiment:
    ...
    def __init__(self, target_maker_args={}, 
                       ...
                       agent_args={}, 
                       experiment_args={}, model=None, ground_truth=False, vision=True):     
...
####################################################################################################################
        ...
        # SOFIAN NOTE : important maybe the results you need are in this file
        #####################################################################
...
#################################################################################################
        ...
        ###########################################################################################################################################################################
        self.train_experience = MultiExperienceMemory(train_experience_args, multi_simulator = self.multi_simulator, target_maker = self.target_maker, model=self.model, ground_truth=self.ground_truth, vision=self.vision)
        ###########################################################################################################################################################################
...
##########################################################################################################################################################################
        self.test_policy_experience = MultiExperienceMemory(test_policy_experience_args, multi_simulator = self.multi_simulator, target_maker = self.target_maker, model=self.model, ground_truth=self.ground_truth, vision=self.vision)
##########################################################################################################################################################################
...
###########################################################################################################################################################################        
        self.ag = FuturePredictorAgentAdvantage(self.sess, agent_args, ground_truth=self.ground_truth, vision=self.vision) 
###########################################################################################################################################################################
    ...
    def run(self, mode):
        if mode == 'show':  
            ...
            rwrd_dict = self.ag.train(self.multi_simulator, self.train_experience, self.num_train_iterations, test_policy_experience=self.test_policy_experience)
            return rwrd_dict
        else:
