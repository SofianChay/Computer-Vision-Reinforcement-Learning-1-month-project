...
##############################################################################
        ...
        # self._game.set_vizdoom_path(os.path.join(vizdoom_path,'bin/vizdoom')) # Sets path to ViZDoom engine executable.
        # self._game.set_doom_game_path(os.path.join(vizdoom_path,'bin/freedoom2.wad')) # Sets path to the Doom engine based 
        # game file (wad format). If not used DoomGame will look for doom2.wad and freedoom2.wad (in that order) in the directory of ViZDoom's installation (where vizdoom.so is).
...
##############################################################################
                ...
                for label in state.labels:
                    if label.value not in self.labels_figures:
                        self.labels_figures.append(label.value)
##############################################################################             
            if self.resize:
...
#########################################################################################################
            labels = np.zeros((self.resolution[1], self.resolution[0]), dtype=np.uint8)
#########################################################################################################
            meas = np.zeros(self.num_meas, dtype=np.uint32) # should ideally put nan here, but since it's an int...

#############################################################################
        return img, labels, meas, rwrd, term
#############################################################################    
